<div class="panel panel-default">
    ...
    <div class="panel-body">
        ...
        @if (isset($location))
            <fieldset class="{{current_route_matches('LocationController@createTranslation') ? 'hide':''}}">
                ...
                <div class="btn-group">
                    @foreach(locales() as $code => $properties)
                        @if ($location->hasTranslation($code))
                            @if (fallback_locale() == $code)
                                <a href="{{action('Admin\LocationController@edit', ['id'=>$location->id])}}"
                    ...
                    @if ($location->hasUntranslatedLocale())
                        <a type="button" class="btn btn-default btn-xs btn-flag"
                           href="{{action('Admin\LocationController@createTranslation',$location->id)}}">
        ...
        <fieldset>
            ...
            <div class="form-group">
                ...
                <select name="locale"
                        class="form-control select2" {{current_route_matches('LocationController@createTranslation') ? '':'disabled'}}>
        ...
        <fieldset class="">
            ...
            <div class="form-group  {{ error_has('name') ? 'has-error':''}} ">
                ...
                <input type="text" class="form-control" name="name" value="{{old('name',  @$location->{"name:$locale"})  }}">
                {!!error_message('name')  !!}
            </div>
        </fieldset>
        ...
        <fieldset>
            <div class="form-group">
                ...
                <textarea id="myTextarea" name="address" rows="5"
                          class="form-control ">{{old('address',  @$location->{"address:$locale"}) }}</textarea>
            </div>
        </fieldset>
        ...
        <fieldset>
            <div class="form-group">
                ...
                <textarea id="myTextarea" name="remark" rows="5"
                          class="form-control ">{{old('remark',  @$location->{"remark:$locale"}) }}</textarea>
            </div>
        </fieldset>
    ...
    <div class="panel-footer">
        <div class="clearfix">
            <div class="pull-left">
                <button type="button"
                        class="btn btn-danger btn-delete {{current_route_matches('LocationController@editTranslation') ? '' : 'hide'}}">
